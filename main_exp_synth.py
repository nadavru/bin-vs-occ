...
data_functions = [create_data.synth_1, create_data.synth_2, create_data.synth_3, create_data.synth_4, create_data.synth_5]
outliers_ratios = [0.002, 0.01, 0.04, 0.2]

all_options = list(product(data_functions, outliers_ratios))
if len(sys.argv)>=2:
...
for (data_func, outliers_ratio) in all_options:
    ...
    inliers_length = total_rows - outliers_length

    in_dataset = data_func(inliers_length, d, in_dist=True)
    ...
    result_folder = f"{results_folder}/{data_name}/{outliers_ratio}"
    ...
    graph_title = f"{data_name}: {outliers_length} ({outliers_perc}%)"
    ...
    out_dataset = data_func(total_rows, d, in_dist=False)
